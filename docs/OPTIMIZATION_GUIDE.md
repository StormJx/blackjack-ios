...
| 2 | L 上架准备 | ~~L1~~ ~~L5 文案/截图/标签稿~~ **已完成** → L3 问卷须在 Connect 勾选；隐私页待托管 URL；L2 分牌仍须先锁产品 | A2 已完成 |
| 3 | T 教学轨 | ~~T2 局末复盘~~ **已取消** → T1 策略引擎 → T3 正确率统计 → T4 实时提示 → T5 算牌训练 | T3/T4 挂 T1；勿再做局末复盘面板 |
...
**规模**：中（1 刀）。这是 T3–T4 的地基（T2 已取消）。  
**状态：已完成（2026-08-30）** — `BasicStrategy.swift` 静态表（多副 S17、晚投降、DAS 对子表）+ `BasicStrategyTests`；零 UI。

### T2 局末决策复盘 — **已取消（2026-08-30）**

产品裁定：本品是干净练习 / 公平闯关游戏，不做局末逐手复盘对照。  
**未接线、无源码、无设置项。** 勿在 `SessionCoordinator` / `SessionRoundEndPanel` / `BlackjackGame` 加决策快照或复盘面板。  
原计划（已作废）：局末可折叠对照 T1、设置可关。若日后要教学反馈，走 T3 正确率或 T4 实时提示，须重新点名。
